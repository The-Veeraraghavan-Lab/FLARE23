...
class UnetBasicBlock_res(nn.Module):
...
class SMIT_3D_Seg_mini(nn.Module):
    def __init__(
        self,
        config,
        out_channels: int ,
        ...
        self.encoder1 = UnetrBasicBlock_No_DownSampling(
            spatial_dims=spatial_dims,
    ...
    def forward(self, x_in):
        ...
        enc1 = self.encoder1(enc11) #input size torch.Size([4, 96, 64, 64, 64])
        
        enc2 = self.encoder2(enc22) #input size torch.Size([4, 192, 32, 32, 32])
       
        enc3 = self.encoder3(enc33) #torch.Size([4, 384, 16, 16, 16])
       
        enc4 = self.encoder4(enc44) 
       
        enc_bottleneck = self.encoder10(x)
        
        dec4 = self.decoder5(enc_bottleneck, enc4)
