...
def sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    importance_map = compute_importance_map(
        get_valid_patch_size(image_size, roi_size), mode=mode, sigma_scale=sigma_scale, device='cpu'
    )

    # Perform predictions
    output_image, count_map = torch.tensor(0.0, device=device), torch.tensor(0.0, device=device)
    _initialized = False
    _initialized_out = False
    for slice_g in tqdm(range(0, total_slices, sw_batch_size)) if progress else range(0, total_slices, sw_batch_size):
        ...
        pred_out = predictor(window_data, *args, **kwargs) # batched patch segmentation
        
        seg_prob = pred_out.to('cpu')
